          ...
          {testimonials.map(({ text, name, role, rating }) => (
            <StaggerItem key={name}>
              <GlassCard className="relative h-full">
                <span className="absolute -top-2 -left-1 text-5xl font-serif leading-none text-jona-blue/20 dark:text-blue-400/20">&ldquo;</span>
                <Stars count={rating} />
                <p className="mt-4 text-sm leading-relaxed text-stone-600 italic md:text-base dark:text-stone-400">
                  {text}
                </p>
                <div className="mt-6">
                  <p className="font-semibold text-jona-blue dark:text-blue-300">{name}</p>
